...
<section class="concerts-section py-5">
    <div class="container">
        <h2 class="text-center mb-5">Upcoming Concerts</h2>

        <!-- فرم جستجو -->
        <form id="search-form" class="mb-4 position-relative">
            <div class="input-group">
                <input type="text" id="search-input" class="form-control" placeholder="Search concerts by name or ...">
...
<script>
    ...
    function displayConcerts(filteredConcerts) {
        ...
        const limitedConcerts = filteredConcerts.slice(0, 4); // محدود به 4 مورد

        if (limitedConcerts.length === 0) {
        ...
        limitedConcerts.forEach(concert => {
            const concertCard = `
                <div class="col-md-3 col-sm-6 mb-4 d-flex align-items-stretch">
                    <div class="concert-card-unique w-100 position-relative">
                        ${activeFields.includes('event_date') ? `
                        <div class="concert-date-unique bg-danger text-white position-absolute d-flex align-items-center justify-content-center">
                            ...
                            ${dateDisplayType === 'Show Date' ? 
                                ...
                                calculateTimeRemaining(concert.event_date)
                            }
                        </div>` : ''}
                        <div class="concert-image-wrapper-unique">
                            <img src="/concert/${concert.banner}" class="card-img-top" alt="Concert Image">
                        </div>
                        <div class="card-body-unique">
                            <h5 class="card-title-unique">${truncateText(concert.name, 25)}</h5>
                            ${activeFields.includes('description') ? `<p class="card-text-unique">${truncateText(concert.description || '', 100)}</p>` : ''}
            ...
            `;
            concertResults.innerHTML += concertCard;
        });
    }
    ...
    function filterConcerts(query) {
        ...
        return concerts.filter(concert =>
            ...
            concert.tags.toLowerCase().includes(lowerQuery)
        );
    }
